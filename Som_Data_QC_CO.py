...
for i in range(0,Qc_Old_record['Mailed_Diff'].count()):
     if Qc_Old_record['Mailed_Diff'].iloc[i] > 2:
          QC_Check_Counter = 1
          QC_Check_Error_Message = QC_Check_Error_Message + "\n Mail difference for DateMailed " + str(Qc_Old_record['DATEMAILED_New'].iloc[i])+ " is greater than expected." + " Mail difference is " + str(Qc_Old_record['Mailed_Diff'].iloc[i])
...
writer = pd.ExcelWriter("C:\\Deep\\Python\\Project_Som_Data\\QC_Result\\Som_Data_QC_Result_CO.xlsx", engine='xlsxwriter')
